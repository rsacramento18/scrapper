...
getPricesFromJumbo = async (obj) => {
  ...
  for (x of obj.items) {
    ...
    if (scrapes.price) {
      price = scrapes.price.replace(/^\s+|\s+$/g, '');
      price = Math.abs(price.substring(0, price.indexOf('€') - 1).replace(',','.'));
    }

    if (scrapes.priceUnit) {
      priceUnit = scrapes.priceUnit.replace(/^\s+|\s+$/g, '');
...
getPricesFromPingoDoce = async (obj) => {
  ...
  for (x of obj.items) {
    ...
    if( scrapes.price) {
      price = Math.abs(scrapes.price.replace(/^\s+|\s+$/g, '').substring(0, scrapes.price.indexOf('€')).replace(',', '.'));
      priceUnit = Math.round((price/x.qty) * 100) / 100;
    }
